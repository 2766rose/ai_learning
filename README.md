...
## 🔬 框架对比实验：自研 ReAct vs LangGraph

为验证「Agent 编排方式是否影响回答质量」，将自研 ReAct 循环用 **LangGraph** 重写一版，通过配置开关 `RAG_AGENT_BACKEND=builtin|langgraph` 切换，用**同一套 50 题评估集**跑两轮对照：

| 指标 | 自研 builtin | LangGraph |
| --- | --- | --- |
| answer_correctness | 0.910 | 0.916 |
| faithfulness | 0.9633 | 0.9633 |
| 陷阱题拒答 | 5/5 | 5/5 |
| 代码行数 | 587 | **246** |
| token 级流式 | 原生支持 | 需额外实现 `astream` |

**结论**：编排方式对回答质量几乎无影响（差异在噪声范围），LangGraph 的主要收益是**代码量减少 58%**，代价是流式输出需额外实现。**质量提升应来自知识库治理与检索优化，而非换框架。**

> 选型建议：编排复杂 / 需要断点恢复 → 用 LangGraph；质量约束强、需精细控制流式与兜底 → 自研更直接；生产可混合（框架管编排 + 自研管质量环节）。
