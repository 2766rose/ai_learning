...
def _get_parser(ext: str):
    ...
    if ext not in _parsers_cache:
        if ext == ".pdf":
            ...
            from ai_rag.parsers.docx_parser import DocxParser
            _parsers_cache[ext] = DocxParser()
        elif ext in (".xlsx", ".xls"):
            from ai_rag.parsers.excel_parser import ExcelParser
            _parsers_cache[ext] = ExcelParser()
        elif ext in (".txt", ".md", ".csv", ".log"):
            from ai_rag.parsers.txt_parser import TxtParser
...
async def ingest_document(file_path: str, metadata: Optional[Dict[str, Any]] = None) -> int:
    ...
    # === ✅ Step 2.5: 入库前去重 ===
    from ai_rag.utils.dedup import deduplicate_chunks
    ...
    # cleanup uploaded file after successful ingest
    try:
        os.remove(file_path)
        logger.info("uploaded file cleaned: %s", file_path)
    except Exception as e:
        logger.warning("cleanup failed: %s | error=%s", file_path, e)
