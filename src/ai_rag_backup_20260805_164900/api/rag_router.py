...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        # ✅ 核心修复：send_task 不抛异常 = 投递成功
        #    不再查询 AsyncResult.state / 队列长度做二次验证
        def _safe_send():
...
async def chat(request: ChatRequest, raw_request: Request):
    ...
    # ✅ 解包元组：(ai_answer, retrieved_knowledge)
    ...
    return ChatResponse(
        ...
        retrieved_knowledge=retrieved_knowledge,  # ✅ 现在正确赋值了
    )
